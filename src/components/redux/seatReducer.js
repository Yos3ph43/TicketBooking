...
const seatReducer = (state = initialState, action) => {
  switch (action.type) {
    case "BOOK_SEAT": {
      ...
      return { ...state, bookingSeats: data };
    }
    case "PAYMENT": {
      const data = state.seats.map((item) => {
        const bookedSeats = item.danhSachGhe.map((seat) => {
          state.bookingSeats.forEach((bookingSeat) => {
            if (bookingSeat.soGhe === seat.soGhe) seat.daDat = true;
          });
          return seat;
        });
        return { ...item, danhSachGhe: bookedSeats };
      });
      return { ...state, bookingSeats: [], seats: data };
    }
    default:
      return state;
  }
